...
<body>
	...
	function copiaDados(id){
		...
		document.getElementById('productQty').value = celulas[1].textContent;
	...
	<table border="1">
		<thead>
			<tr>
				<th><b>ID</b></th>
				<th><b>Quantidade do Produto</b></th>
		...
		<tbody>
			<c:forEach items="${productList}" var="pL">
				<tr id="row_${pL.ID}">
					...
					<td><a href="/remove_fullproduct/${pL.ID}"><button type="submit">Remover</button>
					</a></td>
				</tr>
			</c:forEach>
		</tbody>
	...
	<form action="/editarFullProduct" method="post">
		ID: <input type="text" id="id" name="id" readonly style="color: #AAAAAA" /><br>
		Nome do Produto: <input type="text" id="productQty" name="productQty" /><br>
		Descricao do Produto: <input type="text"	id="productPrice" name="productPrice" /><br>
	...
	<form action="/cadastro_fullproduct" method="post">
		<input type="submit" value="Cadastrar Novo FullProduto" />
	</form>
